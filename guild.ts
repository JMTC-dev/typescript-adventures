// Define what makes a GuildMember using an interface
...
interface Guild {
  ...
  assignQuest: (member: GuildMember, quest: Quest) => void;
  addMember: (member: GuildMember) => void;
  removeMember: (member: GuildMember) => void;
  duesPaid: (member: GuildMember) => void;
}

const adventurersGuild: Guild = {
  ...
  assignQuest: (member: GuildMember, quest: Quest): void => {
    if (
      ...
      console.log(
        `${member.name} (Level ${member.level}) is not high enough level. The quest requirement is Level ${quest.minLevel}`
      );
    } else {
      console.log(
        `${member.name} is not part of the ${adventurersGuild.name}.`
      );
    }
  },
  ...
  addMember: (member: GuildMember): void => {
    if (adventurersGuild.guildMembers.includes(member)) {
      return console.log(
        `${member.name} is already part of ${adventurersGuild.name}.`
      );
    }
    if (member.goldBalance >= adventurersGuild.fee) {
      ...
      console.log(
        `${member.name} has paid the ${adventurersGuild.name} fee of $${adventurersGuild.fee} and is now a member of the guild.`
      );
    } else {
      console.log(
        `${member.name} does not have enough money to pay the ${
          adventurersGuild.name
        } fee of $${adventurersGuild.fee}. ${
          adventurersGuild.fee - member.goldBalance
        } gold remaining.`
      );
    }
  },
  ...
  removeMember: (member: GuildMember): void => {
    if (adventurersGuild.guildMembers.includes(member)) {
      let newGuildMembers = adventurersGuild.guildMembers.filter(
        (guildMember) => guildMember.name != member.name
      );
      member.guild = undefined;
      member.duespaid = false;
      adventurersGuild.guildMembers = newGuildMembers;
      console.log(
        `${member.name} has been removed from the ${adventurersGuild.name}`
      );
    } else {
  ...
  duesPaid: (member: GuildMember): void => {
    if (
      ...
      console.log(
        `${member.name} cannot pay the Adventurer's Guild fee of $${adventurersGuild.fee} and will now be removed from the guild.`
      );
      const newGuildMembers = adventurersGuild.guildMembers.filter(
        (guildMember) => guildMember.name != member.name
      );
      member.guild = undefined;
      member.duespaid = false;
      ...
      adventurersGuild.treasury += adventurersGuild.fee;
      console.log(
        `${member.name} has paid the Adventurer's Guild fee of $${adventurersGuild.fee} and remains in the guild.`
      );
      member.duespaid = true;
    } else {
      console.log(
        `${member.name} has paid all fees for ${adventurersGuild.name}`
      );
    }
  },
};

// Extended interfaces for specific guild roles
interface Warrior extends GuildMember {
  weaponType: string;
  armorClass: number;
  battleCries: string[];
}

interface Mage extends GuildMember {
  magicSchool: string;
  manaPool: number;
  spellBook: string[];
}

interface Rogue extends GuildMember {
...
interface Quest {
  ...
  isAvailable: (memberLevel: number) => boolean;
  complete: (member: GuildMember) => void;
}
...
const dragonSlaying: Quest = {
  title: "Slay the Dragon of Mount Doom",
  description:
    "A fearsome dragon has been terrorizing nearby villages. Put an end to it!",
  difficulty: "Hard",
  reward: 1000,
  location: "Mount Doom",
  minLevel: 10,
  ...
  complete: (member: GuildMember): void => {
    if (
      member.activeQuests?.includes(dragonSlaying) &&
      dragonSlaying.isAvailable(member.level)
    ) {
      console.log(
        `${member.name} has completed the quest "${dragonSlaying.title}"`
      );
      ...
      if (!dragonSlaying.isAvailable(member.level)) {
        console.log(
          `${member.name} (Level ${member.level}) is not high enough level. The quest requirement is Level ${dragonSlaying.minLevel}`
        );
      }
      if (!member.activeQuests?.includes(dragonSlaying)) {
        console.log(
          `You are not assigned to ${dragonSlaying.title}! Speak with your guildmaster.`
        );
      }
    }
  },
...
const cooksAssistant: Quest = {
  ...
  description:
    "A cook for the royal duke needs assistance to complete his cake.",
  difficulty: "Easy",
  reward: 500,
  location: "Eldoria",
  minLevel: 1,
  ...
  complete: (member: GuildMember): void => {
    if (
      ...
      let memberInventory = member.inventory.filter(
        (item) => !cooksAssistant.itemsRequired?.includes(item)
      );
      member.inventory = memberInventory;
      let memberQuests = member.activeQuests?.filter(
        (quest) => quest.title != cooksAssistant.title
      );
      member.activeQuests = memberQuests;
      console.log(
        `${member.name} has completed the quest "${cooksAssistant.title}"`
      );
      member.questsCompleted += 1;
      console.log(`${member.name} earned ${cooksAssistant.reward} gold.`);
    } else {
      if (cooksAssistant.itemsRequired != undefined) {
        let missingQuestItems = cooksAssistant.itemsRequired.filter(
          (item) => !member.inventory.includes(item)
        );
        console.log(
          "You don't have all the required items to complete this quest!"
        );
        console.log(`You are missing: ${missingQuestItems.join(", ")} `);
      }
      if (!member.activeQuests?.includes(cooksAssistant)) {
        console.log(
          `You are not assigned to ${cooksAssistant.title}! Speak with your guildmaster.`
        );
      }
      if (!cooksAssistant.isAvailable(member.level)) {
        console.log(
          `${member.name} (Level ${member.level}) is not high enough level. The quest requirement is Level ${cooksAssistant.minLevel}`
        );
      }
    }
  },
...
const displayMemberInfo = (member: GuildMember): void => {
...
const displayWarriorInfo = (warrior: Warrior): void => {
...
const displayMageInfo = (mage: Mage): void => {
...
const displayRogueInfo = (rogue: Rogue): void => {
...
console.log(
  `Can Thorgar take the quest? ${
    dragonSlaying.isAvailable(thorgar.level) ? "Yes" : "No"
  }`
);
console.log(
  `Can a new recruit take the quest? ${
    dragonSlaying.isAvailable(5) ? "Yes" : "No"
  }`
);
...
console.log(
  `Druun's Quest Log: ${druun.activeQuests
